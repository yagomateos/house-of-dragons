...
interface WildfireEscapeBossProps {
...
function spawnAttack(cfg: DifficultyConfig, now: number): Attack {
  ...
  const kind: AttackKind = roll < 0.4 ? 'orb' : roll < 0.65 ? 'jet' : roll < 0.85 ? 'wave' : 'cache';
...
export function WildfireEscapeBoss({ difficulty, startingHealth, onDamage, onWin, onLose }: WildfireEscapeBossProps) {
  ...
  return (
    <div className="wildfire-game">
      <div className="wildfire-hud">
        <div className="wildfire-health">
          <span className="wildfire-health-icon">❤</span>
          <div className="wildfire-health-track">
            <div
              className={`wildfire-health-fill ${healthPct <= 25 ? 'wildfire-health-fill--low' : healthPct <= 55 ? 'wildfire-health-fill--mid' : ''}`}
              style={{ width: `${healthPct}%` }}
            />
          </div>
          <span className="wildfire-health-value">{Math.round(healthPct)}/100</span>
        </div>
        <div className="wildfire-timer-track">
          <div className="wildfire-timer-fill" style={{ width: `${timePct}%` }} />
        </div>
      </div>
        ...
        {g.attacks.map((atk) => {
          ...
          if (atk.kind === 'jet') {
            return (
              <div
                key={atk.id}
                className={`wildfire-jet ${telegraphing ? 'wildfire-jet--warn' : 'wildfire-jet--active'}`}
          ...
          if (atk.kind === 'wave') {
            return (
              <div
                key={atk.id}
                className={`wildfire-wave ${telegraphing ? 'wildfire-wave--warn' : 'wildfire-wave--active'}`}
                style={{ top: `${atk.y}%` }}
              />
            );
          }
      ...
      <div className="wildfire-controls" aria-hidden="true">
        <div className="wildfire-dpad">
          <button
            className="wildfire-btn wildfire-btn--up"
          ...
          <div className="wildfire-dpad-row">
            <button
